...
//			APTR result = lmsg->lm_Userdata = AsyncIOBase ? (APTR)OpenAsync(file, MODE_READ, 2*4096) : (APTR)Open(file, MODE_OLDFILE);
...
//			len = AsyncIOBase ? ReadAsync((struct AsyncFile *)fh, buf, len) : Read((BPTR)fh, buf, len);
...
/*			if(AsyncIOBase)
					CloseAsync((struct AsyncFile *)fh);
			else*/	Close((BPTR)fh);
		}
		break;
	}
...
/*			STRPTR url = new UBYTE [strlen(pmsg->URL)+20];
			sprintf(url, "%s?{%d}¿", pmsg->URL, data->PageID);
*/
