...
extern "C" void KPutFmt(const char *format, va_list arg);
...
void SetupDebug(void)
{
  ...
  kprintf("** HTMLview.mcc v" LIB_REV_STRING " startup ***********************\n");
  kprintf("Initializing runtime debugging:\n");

	if(GetVar("htmlview.mcc.debug", (UBYTE*)var, sizeof(var), 0) > 0)
	{
		char *tok;
    char *debug = var;
	...
	kprintf("set debug classes/flags (env:htmlview.mcc.debug): %08x/%08x\n", debug_classes, debug_flags);
